...
"""
program to perform circuit analysis of circuits containing only DC independent sources and resistor elements
input :- a .ckt file which contains the spice circuit 
output:- two dictionaries one containing the node voltages and the other containing the branch current through
         the voltage sources
things that are being tested for :-
1) Malformed circuit file
2) Invalid file name
3) Invalid components
4) Parallely connected voltage sourcecs
5) Serires connected current sources
6) Missing data in file
7) Singular matrix
"""
...
def evalSpice(filename):
    ...
    #to store data for making the auxiliary equations 
    aux_list = list()
    aux_eqns = 0
    ...
    # all the lines of the circuit being read and checking if it ends
    if ".end\n" not in storage:
        raise ValueError("Malformed circuit file")
    ...
    while not flag:  # if the above flag is not triggred then the file isnt read
        if storage[i] == ".end\n":  # exit if the circuit ends
            ...
            for p in data:
                if p[0] == "#": #checking for comments 
                    data = data[: data.index(p)]
            ...
            if node1 in node_connections:
                node_connections[node1].append(
                    (node2, component_name)
                )  # adding node 2 to the list of nodes for node 1
            else:
                node_connections[node1] = [(node2, component_name)]

            if node2 in node_connections:
                node_connections[node2].append(
                    (node1, component_name)
                )  # adding node 1 to the list of nodes for node 2
            else:
            ...
            if component_name[0] == "V":
                # getting no of voltage sources to use as auxilary equations
                aux_eqns += 1
    ...
    if aux_eqns > 1:
        for i in aux_list:
            branch = branch_components[i][
                1
            ]  # this will give we the nodes this voltage source is connected to
            for k in aux_list:
                if k != i:
                    node1, node2 = branch_components[k][1]
                    #detecting parallel connections 
                    if (node1, node2) == branch:
                        raise ValueError("Circuit error: no solution")
                    if (node2, node1) == branch:
                        raise ValueError("Circuit error: no solution")
                    
    if 'GND' not in ascending_list_of_nodes:
        raise ValueError("Circuit error: no solution")
    ...
    ascending_list_of_nodes.remove(
        "GND"
    )  # remode the ground node from the list of unknowns
    ...
    for i in ascending_list_of_nodes:  # i is the node we are working with
        ...
        for k in node_connections[i]:
            ...
            data = branch_components[
                component
            ]  # structure of data ['R', ('1', '2'), 1.0]
            ...
            node1, node2 = data[
                1
            ]  # this is the branch where the component is connected
            ...
            if data[0] == "R":
                ...
                if node1 in unknown_list:
                    coefficient_matrix[count_of_equations][
                        unknown_list.index(node1)
                    ] += (m * 1) / data[
                        2
                    ]  # we update the values from zero(default)
                if node2 in unknown_list:
                    coefficient_matrix[count_of_equations][
                        unknown_list.index(node2)
                    ] += (-1 * m) / data[2]
        ...
        if not np.any(
            coefficient_matrix[count_of_equations]
        ):  # returns false if the array is all zeros
            if current_source_flag:  # all current sources entering one branch
                if constant_matrix[count_of_equations] == 0:  # the currents cancel
                    raise ValueError("Circuit error: no solution")
            raise ValueError(
                "Circuit error: no solution"
            )  # raise error if the matrix contains a zero row

        constant_matrix[count_of_equations] += 0  # all of this
        count_of_equations += (
            1  # after completing KCL at this node we populate the next equation
        )
    ...
    if determinant == 0:  # checking the matrix is singular
        raise ValueError("Circuit error: no solution")
    else:
        X = np.linalg.solve(
            coefficient_matrix, constant_matrix
        )  # X has the final solutions
